...
import mutomvo.Mutation.Parser.CDTParser.CDT.metadata.TLibrary;
// TODO: DOCS
...
public abstract class LibraryScanner {
     ...
     * @param parser
     */
    protected abstract void configParser( SourceParserTest parser , File currentFile );

    protected abstract String getLibraryName();

    public TLibrary scan( String sourceCode ) {

        SourceParserTest parser = new SourceParserTest();
    ...
    public TLibrary scan( File folder ) {
        ...
        List<SourceParserTest> runningParser = new LinkedList<SourceParserTest>();
     ...
     */
    protected TLibrary collectMetadata( List<SourceParserTest> parsers ) {

        TLibrary library = new TLibrary(getLibraryName());

        for (SourceParserTest sourceParser : parsers) {
    ...
    protected TLibrary collectMetadata( SourceParserTest sourceParser , TLibrary library ) {
    ...
    private static final class FileScanTask implements Runnable {
        ...
        private SourceParserTest parser;
        private File file;

        public FileScanTask(SourceParserTest parser, Semaphore semaphore, File file) {
            super();
            this.parser = parser;
