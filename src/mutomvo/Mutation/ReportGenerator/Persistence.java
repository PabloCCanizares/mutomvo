...
public class Persistence {
    ...
    public boolean saveTestSuiteFile(File persistenceTestFile, LinkedList<LinkedList<String>> testSuite, LinkedList<Integer> paramNumberList)
     {
         boolean bRet;
         ...
         if(testSuite!= null && persistenceTestFile != null)
         {
            ...
            try {            
                ...
                //Save the parameters number order
                for(int i=0;i<paramNumberList.size();i++)
                {
                    nValue =  paramNumberList.get(i);
                    strBuff.append(String.valueOf(nValue));
                    if(i < paramNumberList.size()-1)
                        strBuff.append(",");
